...
void Game::Draw()
{
    ...
    if (help)
    {
        ...
        mesh_panel->Add(quad);
        AddBorder(mesh_border.Get(), quad, LIB_COLOR_BLACK);
    ...
    else
    {
            ...
            for (int i = 0; i < FIGURE_WIDTH; i++)
            {
                for (int j = 0; j < FIGURE_HEIGHT; j++)
                {
                    if (figure.blocks[i][j].filled)
                    {
                        ...
                        AddBlock(mesh_blocks.Get(), x, y, figure.blocks[i][j].color);
                    }
                }
            }
        }
                ...
                for (int i = 0; i < FIGURE_WIDTH; i++)
                {
                    for (int j = 0; j < FIGURE_HEIGHT; j++)
                    {
                        if (figure.blocks[i][j].filled)
                        {
                            float x = BLOCK_SIZE * (i + figure.pos.x) + X_POS;
                            float y = BLOCK_SIZE * (j + figure.pos.y) + Y_POS;
                            AddBlock(mesh_blocks.Get(), x, y, figure.blocks[i][j].color, HELPER_COLOR);
                        }
                    }
                }
            }
        ...
        for (int i = 0; i < FIELD_WIDTH; i++)
        {
            for (int j = 0; j < FIELD_HEIGHT; j++)
            {
                if (field[i][j].filled)
                {
                    AddBlock(mesh_blocks.Get(), X_POS + BLOCK_SIZE * i, Y_POS + BLOCK_SIZE * j, field[i][j].color);
                }
            }
        }

        // Game field border
        AddBorder(mesh_border.Get(), libQuad(libVertex(X_POS, Y_POS),
        ...
        for (int i = 0; i < FIGURE_WIDTH; i++)
        {
            for (int j = 0; j < FIGURE_HEIGHT; j++)
            {
                if (g_figures[figure.numNext][0][j][i])
                {
                    ...
                    AddBlock(mesh_blocks.Get(), x, y, libCast<Block::Color>(figure.numNext));
                }
            }
        }

        AddBorder(mesh_border.Get(), libQuad(libVertex(WIDTH - BLOCK_SIZE * 5, Y_POS),
                           libVertex(WIDTH - BLOCK_SIZE, Y_POS + BLOCK_SIZE)), LIB_COLOR_BLACK);
        
        AddBorder(mesh_border.Get(), libQuad(libVertex(WIDTH - BLOCK_SIZE * 5, Y_POS + BLOCK_SIZE),
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            AddBorder(mesh_border.Get(), libQuad(libVertex(WIDTH - BLOCK_SIZE * 5, Y_POS + BLOCK_SIZE * 6 + offsetY),
...
===================
*/
void Game::AddBlock(libMesh *buffer, float x, float y, Block::Color colorType, const libColor &clr) const
{
...
===================
*/
void Game::AddBorder(libMesh *buffer, const libQuad &quad, const libColor &clr) const
{
